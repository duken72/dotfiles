...
pacstrap /mnt base base-devel linux-lts linux-firmware sof-firmware
          iw iwd vi vim grub efibootmgr openssh
```
...
Create config files, with name of device from networkctl list.
...
15. Drivers and packages installation:
```bash
...
16. Post-installation: Setting up desktop environment
Example videos: [1](https://youtu.be/DAmXKDJ3D7M), [2](https://youtu.be/eHdP4sT7-8U), [3](https://youtu.be/FudOL0-B9Hs).
```bash
systemctl enable lightdm
systemctl list-unit-files --state=enabled
reboot
```
Go to archlinux.org/mirrorlist, choose only https, not http, just IPv4, not IPv6, use mirror status.
